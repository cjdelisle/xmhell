module.exports = (function() {
            ...
            for (; i > 0; i = data.indexOf('<', i)+1) {
                switch (data[i]) {
                    ...
                    default:
                        ...
                        if (tag[tag.length-1] === '/') {
                            cont = Util.addTag(cont, tag.substring(0,tag.length-1), '');
                            continue;
                        }
                        i = data.indexOf('<', i+tag.length)+1;
                        cont = Util.addTag(cont, tag, p());
                }
            }
            return cont;
        };
    ...
    var writeSpecial = function (w, key, keyval) {
        var write = function() { w.write(String(keyval.shift())) };
        switch (key) {
            case "#COMMENT": w.write("<!--"); write(); w.write("-->\n"); break;
            case "#CDATA":   w.write("<![CDATA["); write(); w.write("]]>\n"); break;
            case "#DOCTYPE": w.write("<!DOCTYPE "); write(); w.write(">\n"); break;
